...
bool NMIoTAPIClass::download_icon(int coin_id){ 
    ...
    if (!buf) {
        LOG_E("malloc failed for %d bytes", size);
        http.endRequest();
        return false;
    }
    ...
    while(true){
        ...
        if(thread_cnt % 60 == 0) {
            ...
            for(auto &coin : g_nm.coin_map) {
                LOG_W("Free heap before: %u", ESP.getFreeHeap());
                api->download_icon(coin.second.id); // Download the icon for the coin
                LOG_W("Free heap after: %u", ESP.getFreeHeap());
                delay(500);                         // Add a small delay to avoid overwhelming the API
            }
        }
